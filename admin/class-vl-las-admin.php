...
public function print_fallback_loader() {
    ...
    public function soc2_help() {
        ...
        <div class="notice notice-info" style="margin:10px 0;">
            ...
            <ul style="margin-left:20px; list-style:disc;">
                <li><?php esc_html_e( 'Ensure your Corporate License Code is saved so the plugin can authenticate with the VL Hub (sent via the X-Luna-License header and license query parameter).', 'vl-las' ); ?></li>
                <li><?php esc_html_e( 'Request your tenant-specific SOC 2 snapshot endpoint from Visible Light (base path: https://hub.visiblelight.ai/wp-json/vl-hub/v1/profile).', 'vl-las' ); ?></li>
    ...
    public function sanitize_soc2_endpoint( $val ) {
        ...
        $default   = defined( 'VL_LAS_SOC2_ENDPOINT_DEFAULT' )
            ? VL_LAS_SOC2_ENDPOINT_DEFAULT
            : 'https://hub.visiblelight.ai/wp-json/vl-hub/v1/profile';
        ...
        if ( empty( $parts['path'] ) || ( false === strpos( $parts['path'], '/vl-hub/' ) && false === strpos( $parts['path'], '/soc2' ) ) ) {
            add_settings_error(
                'vl_las_soc2_endpoint',
                'vl_las_soc2_endpoint_path',
                __( 'The SOC 2 endpoint should point to the VL Hub profile API (e.g. /wp-json/vl-hub/v1/profile).', 'vl-las' )
            );
            return $previous;
        }
        ...
        if ( '' !== $license ) {
            ...
            add_settings_error(
                'vl_las_soc2_endpoint',
                'vl_las_soc2_endpoint_license',
                __( 'Save your Corporate License Code to authenticate requests to the VL Hub.', 'vl-las' )
            );
        }

        $request_url = $url;
        if ( class_exists( '\\VL_LAS_SOC2' ) ) {
            $request_url = \VL_LAS_SOC2::prepare_request_url( $url, $license );
        } else {
            if ( $license && false === strpos( $request_url, 'license=' ) ) {
                $request_url = add_query_arg( 'license', $license, $request_url );
            }
            if ( false === strpos( $request_url, 'refresh=' ) ) {
                $request_url = add_query_arg( 'refresh', '1', $request_url );
            }
        }
        ...
        printf(
            ...
            wp_kses_post(
                sprintf(
                    ...
                    __( 'Example endpoint: %s (replace the license value with the key assigned to your tenant).', 'vl-las' ),
                    '<code>https://hub.visiblelight.ai/wp-json/vl-hub/v1/profile?license=LIC_xxxx&refresh=1</code>'
                )
            )
        );
